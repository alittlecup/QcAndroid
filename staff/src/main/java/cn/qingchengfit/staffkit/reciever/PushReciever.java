...
public class PushReciever extends PushMessageReceiver {
  ...
  @Override
  public void onNotificationClicked(final Context context, String s, String s1, String s2) {
    ...
    if (!TextUtils.isEmpty(s2)) {
      ...
      restRepository.getStaffAllApi().qcClearAllNoti(App.staffId, params);
      if (TextUtils.isEmpty(bean.url)) {
        if (bean.type != 0 && bean.type < 16) {
          final CoachService coachService =
              gymBaseInfoAction.getGymByShopIdNow(bean.brand_id, bean.shop_id);
          ...
          intent.putExtra(Configs.EXTRA_GYM_STATUS,
              new GymStatus.Builder().isSin(false).isGuide(false).isSingle(false).build());
          ...
          switch (bean.type) {
            ...
            case ConstantNotification.TYPE_FITNESS_REMIND_CARD_BALANCE:
              ...
              RouterUtils.routeTo(context, "card", "/detail/",
                  CardDetailParams.builder().cardid(bean.brand_id).build(), intent);
              break;
          }
        ...
        try {
          ...
          final Intent toActivity = new Intent(context.getPackageName(), Uri.parse(bean.url));
          String staffid = PreferenceUtils.getPrefString(context, Configs.PREFER_WORK_ID, "");
                    ...
                    @Override public void call(QcResponsePermission qcResponse) {
                      if (ResponseConstant.checkSuccess(qcResponse)) {
                        ...
                        toActivity.putExtra(Configs.EXTRA_BRAND,
                            new Brand.Builder().id(bean.brand_id).build());
