...
public class MutiChooseCoachActivity extends BaseActivity
  ...
  private void setToolbar() {
    ...
    if (!CompatUtils.less21() && mToolbar.getParent() instanceof ViewGroup) {
      ((ViewGroup) mToolbar.getParent()).setPadding(0, MeasureUtils.getStatusBarHeight(this), 0, 0);
    }
      ...
      @Override public boolean onMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.action_complete) {
          ...
          for (int i = 0; i < mFlexAdapter.getSelectedPositions().size(); i++) {
            AbstractFlexibleItem flexibleItem =
                mDatas.get(mFlexAdapter.getSelectedPositions().get(i));
            if (flexibleItem != null) {
              Staff coachBean = ((ChooseCoachItem) flexibleItem).getData();
              ...
              if (coachBean.getTeacher() != null) {
                mChoosedCoachsList.add(coachBean.getTeacher().getId());
              } else {
          ...
          if (getIntent().getBooleanExtra("hasReturn", false)) {
            ...
            if (mPageType != CHANGE && mChoosedCoachsList.size() == 0) {
              ...
              return true;
            } else if (mPageType > 0 && !TextUtils.isEmpty(coachId) && !mChoosedCoachsList.contains(
                coachId)) {
