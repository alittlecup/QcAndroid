...
import cn.qingchengfit.staffkit.databinding.FragmentAllotsaleSaledetailBinding;
...
public class SaleDetailFragment extends FilterCommonFragment implements SaleDetailPresenter.PresenterView, View.OnClickListener {
    ...
    FragmentAllotsaleSaledetailBinding binding;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_allotsale_saledetail, container, false);
        setView(binding.getRoot());
        super.onCreateView(inflater, container, savedInstanceState);
        if (saler.getStaff() == null) return binding.getRoot();
        mCallbackActivity.setToolbar(TextUtils.isEmpty(saler.getStaff().username) ? getString(R.string.qc_allotsale_sale_detail_notitle)
                : getString(R.string.qc_allotsale_sale_detail_title, saler.getStaff().username), false, null,
            TextUtils.isEmpty(saler.getStaff().username) ? R.menu.menu_multi_allot : R.menu.menu_multi_modify,
            new Toolbar.OnMenuItemClickListener() {

                @Override public boolean onMenuItemClick(MenuItem item) {
                    // 跳转批量修改
                    MultiModifyFragment multiModifyFragment = MultiModifyFragmentBuilder.newMultiModifyFragment(saler.getStaff(), filter,
                        TextUtils.isEmpty(saler.getStaff().username) ? MultiModifyFragment.TYPE_MULTI_ALLOT
    ...
    private void initView() {
        ...
        binding.smrvSale.setLayoutManager(mLinearLayoutManager);
        binding.smrvSale.addItemDecoration(new DividerItemDecoration(getContext(), LinearLayoutManager.VERTICAL));
        binding.smrvSale.setAdapter(studentAdapter);
        ...
        if (TextUtils.isEmpty(saler.getStaff().username)) {
            ...
            binding.llAddStu.setOnClickListener(this);
        }

        // 侧边索引点击监听,关闭item的左滑菜单
        binding.alphabetview.setOnTouchListener(new View.OnTouchListener() {
            @Override public boolean onTouch(View v, MotionEvent event) {
                return false;
            }
        });
        // 侧边索引选中监听,list滚动到指定位置
        binding.alphabetview.setOnAlphabetChange(new AlphabetView.OnAlphabetChange() {
            @Override public void onChange(int position, String s) {
                if ("#".equals(s)) s = "~";
                mLinearLayoutManager.scrollToPositionWithOffset(studentAdapter.getPositionForSection(s.charAt(0)), 0);
            }
        });
    ...
    @OnClick public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.ll_add_stu:
                MultiModifyFragment fragment = newMultiModifyFragment(saler.getStaff(), filter, MultiModifyFragment.TYPE_ADD_STUDENT);
    ...
    public class DetailStudentAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
        ...
        @Override public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            if (viewType == TYPE_NORMAL) {
                holder = new MyBindingViewHolder(
                    LayoutInflater.from(parent.getContext()).inflate(R.layout.item_allotsale_detail_student, parent, false));
                RxBusAdd(EventFreshStudent.class).subscribe(new Action1<EventFreshStudent>() {
                    @Override public void call(EventFreshStudent eventFreshStudent) {
                        freshData();
                    }
                ...
                holder = new StudentAdapter.ItemViewHolder(
                    LayoutInflater.from(parent.getContext()).inflate(R.layout.item_total_count_footer, parent, false));
                // 注册 event 刷新列表
                RxBusAdd(EventFreshStudent.class).subscribe(new Action1<EventFreshStudent>() {
                    @Override public void call(EventFreshStudent eventFreshStudent) {
                        freshData();
        ...
        @Override public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
            ...
            if (holder instanceof MyBindingViewHolder) {
                ...
                if (studentBean.sellers == null || studentBean.sellers.size() == 0) {
                    ...
                    ((MyBindingViewHolder) holder).itemPersonDesc.setText(
                        holder.itemView.getContext().getString(R.string.qc_allotsale_sale, StringUtils.sellersNames(studentBean.sellers)));
                }
                ...
                if (studentBean.gender) {//男
                    ((MyBindingViewHolder) holder).itemPersonGender.setImageResource(R.drawable.ic_gender_signal_male);
                } else {
                    ((MyBindingViewHolder) holder).itemPersonGender.setImageResource(R.drawable.ic_gender_signal_female);
                }
                ...
                Glide.with(holder.itemView.getContext())
                    ...
                    .into(new CircleImgWrapper(((MyBindingViewHolder) holder).itemPersonHeader, holder.itemView.getContext()));
            } else {
