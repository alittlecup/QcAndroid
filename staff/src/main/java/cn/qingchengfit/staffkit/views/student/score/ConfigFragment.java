...
public class ConfigFragment extends BaseFragment implements ConfigPresenter.PresenterView, FlexibleAdapter.OnItemClickListener {
    ...
    private void initView() {
        ...
        recyclerviewAward.addItemDecoration(new DividerItemDecoration(getContext(), LinearLayoutManager.VERTICAL));
        ...
        recyclerviewAwardDis.addItemDecoration(new DividerItemDecoration(getContext(), LinearLayoutManager.VERTICAL));
        ...
        if (isLoading && (scoreRule == null || scoreRule.allDisable())) {
            ...
            swtScoreConfig.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
    ...
    @Override public void onScoreRules(ScoreRule scoreRule) {
        ...
        if (scoreRule.buycard_enable
            ...
            tvScoreRuleUpdateInfo.setText(new StringBuilder().append("修改时间 ")
                .append((!TextUtils.isEmpty(scoreRule.updated_at)) ? scoreRule.updated_at.replace("T", " ") : " ")
                .append(" 由")
                .append((scoreRule.updated_by != null && !TextUtils.isEmpty(scoreRule.updated_by.username)) ? scoreRule.updated_by.username
                    : " ")
                .append("操作")
                .toString());
            ...
            if (scoreRule.buycard_enable != null && scoreRule.buycard_enable && scoreRule.buycard != null && !scoreRule.buycard.isEmpty()) {
                StudentScoreBaseConfigBean baseConfigBean = new StudentScoreBaseConfigBean();
    ...
    @Override public boolean onItemClick(int position) {
        ...
        getFragmentManager().beginTransaction()
            .replace(mCallbackActivity.getFragId(),
                ScoreAwardAddFragment.modify(this, 22, scoreAwardRuleBeanListActive.get(position), others))
            .addToBackStack(null)
            .commit();
        return false;
    }
    ...
    @OnClick({ R.id.ll_config_base, R.id.ll_config_award_add }) public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.ll_config_award_add:
                ...
                getFragmentManager().beginTransaction()
                    .replace(mCallbackActivity.getFragId(),
                        ScoreAwardAddFragment.newInstance(this, 11, (ArrayList<StudentScoreAwardRuleBean>) scoreAwardRuleBeanListActive))
                    .addToBackStack(null)
                    .commit();
                break;
        }
