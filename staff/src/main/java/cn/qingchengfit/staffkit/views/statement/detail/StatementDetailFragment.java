 ...
 */
public class StatementDetailFragment extends BaseFragment implements StatementDetailView, OnRecycleItemClickListener {
    ...
    private List<QcResponseStatementDetail.StatamentSchedule> mAllSchedule = new ArrayList<>();//全部报表数据
    //筛选条件
    ...
    private Toolbar.OnMenuItemClickListener MenuClick = new Toolbar.OnMenuItemClickListener() {
        @Override public boolean onMenuItemClick(MenuItem item) {
            ...
            getFragmentManager().beginTransaction()
                .replace(mCallbackActivity.getFragId(), OutExcelFragment.newInstance(mFilter, mChooseShopId))
    ...
    public static StatementDetailFragment newInstance(int type, String starttime, String endtime, String model, int sysId,
        String teacher_id, String courseId, String course_extra, ClassStatmentFilterBean bean) {
        Bundle args = new Bundle();
        args.putInt("type", type);
            ...
            if (requestCode == 1) {
                ...
                if (TextUtils.isEmpty(mChooseShopId)) {
                    mCallbackActivity.setBar(new ToolbarBean(mTitle, true, new View.OnClickListener() {
                        @Override public void onClick(View v) {
                            ChooseGymActivity.start(StatementDetailFragment.this, 1, PermissionServerUtils.COST_REPORT,
    ...
    @Override public void onSuccess(List<QcResponseStatementDetail.StatamentSchedule> list) {
        ...
        for (int i = 0; i < mAllSchedule.size(); i++) {
            ...
            //可供筛选的条件
            if (!mFilterCoaches.contains(statamentSchedule.teacher)) mFilterCoaches.add(statamentSchedule.teacher);
            if (!mFilterCourse.contains(statamentSchedule.course)) mFilterCourse.add(statamentSchedule.course);
    ...
    private void filter() {
        ...
        for (int i = 0; i < mAllSchedule.size(); i++) {
            ...
            if ((mFilter.course_type == -1 || mFilter.course_type == 99 || ((mFilter.course_type == -2
                && statamentSchedule.course.is_private()) || (mFilter.course_type == -3 && !statamentSchedule.course.is_private()))) && (
                mFilter.course == null
                ...
                || (statamentSchedule.getUsersIds().contains(mFilter.student.getId()))) && (DateUtils.formatDateFromYYYYMMDD(mFilter.start)
                        ...
                        if (order.channel.equalsIgnoreCase(Configs.CHANNEL_CARD)) {
                            ...
                            switch (order.card.card_type) {
                                case Configs.CATEGORY_VALUE:
                                    ...
                                    caculator.put("value_real_income", caculator.get("value_real_income") + order.total_real_price);
                                    caculator.put("value_course_income", caculator.get("value_course_income") + order.total_price);
                                    break;
                            ...
                            caculator.put("online_real_income", caculator.get("online_real_income") + order.total_real_price);
                            caculator.put("online_course_income", caculator.get("online_course_income") + order.total_price);
                        }
                    }
                }
                caculator.put("time_course_income", caculator.get("time_course_income") + statamentSchedule.total_times);
            }
        }

        List<CourseTypeform> courseTypeforms = new ArrayList<>();
        CourseTypeform form1 = new CourseTypeform(Configs.TYPE_PRIVATE, TypeUtils.Float2Int(caculator.get("private_count")),
            TypeUtils.Float2Int(caculator.get("private_server_count")));
        CourseTypeform form2 = new CourseTypeform(Configs.TYPE_GROUP, TypeUtils.Float2Int(caculator.get("group_count")),
            TypeUtils.Float2Int(caculator.get("group_server_count")));
        CourseTypeform form = new CourseTypeform(-1, form1.course_count + form2.course_count, form1.server_count + form2.server_count);
        ...
        CourseCardForm courseCardForm1 = new CourseCardForm(Configs.CATEGORY_VALUE, TypeUtils.Float2Int(caculator.get("value_ser_num")),
            caculator.get("value_course_income"), caculator.get("value_real_income"));
        CourseCardForm courseCardForm2 = new CourseCardForm(Configs.CATEGORY_TIMES, TypeUtils.Float2Int(caculator.get("time_ser_num")),
            caculator.get("time_course_income"), caculator.get("time_real_income"));
        CourseCardForm courseCardForm3 =
            new CourseCardForm(Configs.CATEGORY_DATE, TypeUtils.Float2Int(caculator.get("date_ser_num")), 0, 0);
        CourseCardForm courseCardForm4 =
            new CourseCardForm(0, TypeUtils.Float2Int(caculator.get("online_ser_num")), caculator.get("online_course_income"),
                caculator.get("online_real_income"));
        CourseCardForm courseCardForm = new CourseCardForm(-1,
            courseCardForm1.server_count + courseCardForm2.server_count + courseCardForm3.server_count + courseCardForm4.server_count, 0,
        ...
        CourseTypeFormFragment courseTypeFormFragment = CourseTypeFormFragment.newInstance(courseTypeforms);
        CourseCardFormFragment courseCardFormFragment = CourseCardFormFragment.newInstance(courseCardForms);
        fs.clear();
    ...
    @Override public void onItemClick(View v, int pos) {
        getFragmentManager().beginTransaction()
            .replace(mCallbackActivity.getFragId(), CourseReverseFragmentBuilder.newCourseReverseFragment(statementBeans.get(pos).id))
            .addToBackStack(null)
            .commit();
    }
    ...
    class StatementDetailAdapter extends RecyclerView.Adapter<StatementDetailVH> implements View.OnClickListener {
        ...
        @Override public StatementDetailVH onCreateViewHolder(ViewGroup parent, int viewType) {
            StatementDetailVH holder =
                new StatementDetailVH(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_statement_detail, parent, false));
        ...
        @Override public void onBindViewHolder(StatementDetailVH holder, int position) {
            ...
            holder.imageCourseType.setImageResource(
                bean.course.is_private ? R.drawable.ic_course_private_conner : R.drawable.ic_course_group_conner);
            holder.itemStatementDetailName.setText(bean.course.getName() + "-" + bean.teacher.username);
            holder.itemStatementDetailContent.setText(
                DateUtils.getTimeHHMM(date) + "-" + DateUtils.getTimeHHMM(endDate) + "  " + bean.shop.name);
