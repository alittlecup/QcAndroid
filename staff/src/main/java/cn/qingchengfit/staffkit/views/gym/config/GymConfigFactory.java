...
public class GymConfigFactory {
    @Nullable public static HintExpandItem getConfitItem(String key, SignInConfig.Config config, Context context) {
        switch (key) {
            case ShopConfigs.PRIVATE_BEFORE_REMIND_USER_MINUTES:
                return new HintExpandItem(config, null, true, true, "私教开始前提醒会员", "课程开始前（分钟）");
            case ShopConfigs.TEAM_BEFORE_REMIND_USER_MINUTES:
                return new HintExpandItem(config, null, true, true, "团课开始前提醒会员", "课程开始前（分钟）");
