  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    if (mSaleFilter != null) {
      ...
      if (mSaleFilter.card != null) {
        ...
        } else if (mSaleFilter.card_category == Configs.CATEGORY_TIMES) {
          cardType.setContent(getString(R.string.cardtype_times));
        } else if ("ALL".equals(mSaleFilter.order_extra)) {
          cardType.setContent("单次支付");
        } else if ("WEIXIN".equals(mSaleFilter.order_extra)) {
  ...
  public void onClick(View view) {
    switch (view.getId()) {
      ...
      case R.id.custom_statment_generate:
        if (DateUtils.formatDateFromYYYYMMDD(startDay.getContent()).getTime()
            > DateUtils.formatDateFromYYYYMMDD(endDay.getContent()).getTime()) {
