...
public class AddBatchFragment extends BaseFragment implements AddBatchView {
  ...
  private cn.qingchengfit.widgets.DialogList openDialog;
  ...
  public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container,
    ...
    if (mCourse != null) {
      ...
      Glide.with(getContext())
          .load(PhotoUtils.getSmall(mCourse.photo))
      ...
      Glide.with(getContext())
          ...
          .asBitmap()
          .placeholder(mTeacher.gender == 0 ? R.drawable.default_manage_male
    ...
    recyclerview.addItemDecoration(
        new DividerItemDecoration(getContext(), LinearLayoutManager.VERTICAL,
            MeasureUtils.dpToPx(8f, getResources())));
  ...
  @Override public boolean onFragmentBackPress() {
    if (exitDialg == null) {
      exitDialg = DialogUtils.instanceDelDialog(getContext(),
          mType == Configs.TYPE_PRIVATE ? "确定放弃本次排期？" : "确定放弃本次排课",
  ...
  @Override
  public void onTemplete(boolean isFree, ArrayList<Rule> rules, ArrayList<Time_repeat> time_repeats,
      int maxuer) {
    if (gymWrapper.isPro()) {
      // 可以选择不自动填充
  ...
  @OnClick(R.id.order_sutdent_count) public void clickStuCount() {
    stucount = new DialogList(getContext()).list(
        mType == Configs.TYPE_PRIVATE ? StringUtils.getNums(1, 10) : StringUtils.getNums(1, 300),
  ...
  @OnClick(R.id.pay_online) public void clickPayOnline() {
    Intent toPayOnline =
        IntentUtils.getChooseIntent(getContext(), null, null, ChooseActivity.BATCH_PAY_ONLINE);
  ...
  @OnClick(R.id.pay_card) public void clickPayCard() {
    Intent toPayOnline = IntentUtils.getChooseIntent(getContext(), gymWrapper.getCoachService(),
        gymWrapper.getBrand(), ChooseActivity.BATCH_PAY_CARD);
