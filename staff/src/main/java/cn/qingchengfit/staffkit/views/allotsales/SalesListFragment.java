...
import cn.qingchengfit.staffkit.databinding.FragmentAllotsaleSalelistBinding;
...
public class SalesListFragment extends BaseFragment implements SalesListPresenter.PresenterView {
    ...
    FragmentAllotsaleSalelistBinding binding;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_allotsale_salelist, container, false);
    ...
    private void initView() {
        ...
        binding.rvSales.setLayoutManager(mLinearLayoutManager);
        binding.rvSales.addItemDecoration(new DividerItemDecoration(getContext(), LinearLayoutManager.VERTICAL));
        binding.rvSales.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override public void onRefresh() {
                binding.rvSales.setFresh(false);
            }
        });
        datas.clear();
    ...
    public class AllotSalesAdapter extends RecyclerView.Adapter<MyBindingViewHolder> implements View.OnClickListener {
        ...
        @Override public MyBindingViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            MyBindingViewHolder holder =
                new MyBindingViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_allotsale_sale, parent, false));
        ...
        @Override public void onBindViewHolder(MyBindingViewHolder holder, int position) {
            ...
            holder.tvSaleName.setText(
                TextUtils.isEmpty(seller.username) ? getResources().getString(R.string.qc_allotsale_sale_detail_notitle) : seller.username);
            if (TextUtils.isEmpty(seller.username)) {
                holder.itemSalesHeader.setImageDrawable(getResources().getDrawable(R.drawable.ic_nosales_normal));
            }
            holder.tvSaleStucount.setText(getString(R.string.qc_allotsale_stucount, salePreView.getCount()));

            String headerUrl = seller.getAvatar();
            if (!TextUtils.isEmpty(seller.username) && headerUrl != null && headerUrl.equals("")) {
                if (seller.gender == 0) {
                    holder.itemSalesHeader.setImageDrawable(getResources().getDrawable(R.drawable.default_manage_male));
                } else {
                    holder.itemSalesHeader.setImageDrawable(getResources().getDrawable(R.drawable.default_manager_female));
                }
        ...
        @Override public void onClick(View v) {
            if (getActivity() != null && getActivity() instanceof AllotSalesActivity) {
                ((AllotSalesActivity) getActivity()).staffWrapper.setStaff(adapterDatas.get((int) v.getTag()).getSeller());
            }
            // 跳转到分配销售
