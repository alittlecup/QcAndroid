...
public class QcTagContainerLayout extends ViewGroup {
    ...
    /**
     * QcTagContainerLayout border width(default 0.5dp)
     */
    private float mBorderWidth = 0.5f;
    /**
     * QcTagContainerLayout border radius(default 10.0dp)
     */
    ...
    /**
     * QcTagContainerLayout border color(default #22FF0000)
     */
    private int mBorderColor = Color.parseColor("#22FF0000");
    /**
     * QcTagContainerLayout background color(default #11FF0000)
     */
     ...
     */
    private QcTagView.OnTagClickListener mOnTagClickListener;
    ...
    public QcTagContainerLayout(Context context) {
        this(context, null);
    }

    public QcTagContainerLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public QcTagContainerLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mVerticalInterval = (int) attributes.getDimension(R.styleable.AndroidTagView_vertical_interval, dp2px(context, DEFAULT_INTERVAL));
        mHorizontalInterval =
            (int) attributes.getDimension(R.styleable.AndroidTagView_horizontal_interval, dp2px(context, DEFAULT_INTERVAL));
        mBorderWidth = attributes.getDimension(R.styleable.AndroidTagView_container_border_width, dp2px(context, mBorderWidth));
        mBorderRadius = attributes.getDimension(R.styleable.AndroidTagView_container_border_radius, dp2px(context, mBorderRadius));
        mTagBdDistance = attributes.getDimension(R.styleable.AndroidTagView_tag_bd_distance, mTagBdDistance);
        ...
        mSensitivity = attributes.getFloat(R.styleable.AndroidTagView_container_drag_sensitivity, mSensitivity);
        ...
        mTagBorderWidth = attributes.getDimension(R.styleable.AndroidTagView_tag_border_width, dp2px(context, mTagBorderWidth));
        mTagBorderRadius = attributes.getDimension(R.styleable.AndroidTagView_tag_corner_radius, dp2px(context, mTagBorderRadius));
        mTagHorizontalPadding = (int) attributes.getDimension(R.styleable.AndroidTagView_tag_horizontal_padding, mTagHorizontalPadding);
        mTagVerticalPadding = (int) attributes.getDimension(R.styleable.AndroidTagView_tag_vertical_padding, mTagVerticalPadding);
        mTagTextSize = attributes.getDimension(R.styleable.AndroidTagView_tag_text_size, sp2px(context, mTagTextSize));
        mTagBorderColor = attributes.getColor(R.styleable.AndroidTagView_tag_border_color, mTagBorderColor);
        mTagBackgroundColor = attributes.getColor(R.styleable.AndroidTagView_tag_background_color, mTagBackgroundColor);
    ...
    @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int lines = childCount == 0 ? 1 : getChildLines(childCount);
        ...
        if (childCount == 0) {
            ...
            setMeasuredDimension(widthSpecSize,
                (mVerticalInterval + mChildHeight) * lines - mVerticalInterval + getPaddingTop() + getPaddingBottom());
        } else {
    ...
    @Override protected void onLayout(boolean changed, int l, int t, int r, int b) {
            ...
            if (childView.getVisibility() != GONE) {
                ...
                if (mGravity == Gravity.RIGHT) {
                    ...
                    if (curLeft + width - getPaddingLeft() > availableW) {
                        int leftW = getMeasuredWidth() - mViewPos[(i - 1) * 2] - getChildAt(i - 1).getMeasuredWidth() - getPaddingRight();
                    ...
                    if (i == childCount - 1) {
                        int leftW = getMeasuredWidth() - mViewPos[i * 2] - childView.getMeasuredWidth() - getPaddingRight();
        ...
        for (int i = 0; i < mViewPos.length / 2; i++) {
            ...
            childView.layout(mViewPos[i * 2], mViewPos[i * 2 + 1], mViewPos[i * 2] + childView.getMeasuredWidth(),
    ...
    private int getChildLines(int childCount) {
        ...
        for (int i = 0, curLineW = 0; i < childCount; i++) {
            ...
            int height = childView.getMeasuredHeight() < MeasureUtils.dpToPx(26f, getResources()) ? MeasureUtils.dpToPx(26f, getResources())
    ...
    private void onSetTag() {
        mChildViews.add(0, headerView);
        ViewGroup.LayoutParams params = new LayoutParams(LayoutParams.WRAP_CONTENT, MeasureUtils.dpToPx(26f, getResources()));
        addView(headerView, 0);
        if (mTags == null || mTags.size() == 0) {
            return;
        }
        int maxCount = mTags.size() > 15 ? 15 : mTags.size();
    ...
    private void processPreselectedRender(int position, QcTagView tag) {
        if (mMode == LayouMode.MULTIPLE_CHOICE || mMode == LayouMode.SINGLE_CHOICE) {
    ...
    private void onAddTag(String text, int position) {
        ...
        QcTagView tagView = new QcTagView(getContext(), text);
        initTagView(tagView);
        ...
        addView(tagView, position, new LayoutParams(LayoutParams.WRAP_CONTENT, MeasureUtils.dpToPx(36f, getResources())));
    }

    private void initTagView(QcTagView tagView) {
    ...
    private void onRemoveTag(int position) {
        ...
        if (position >= 1 && position < mTags.size() + 1) mTags.remove(position - 1);
        removeAllTags();
        onSetTag();
    ...
    @Override public void postInvalidate() {
        if (mTags != null && mChildViews != null && mChildViews.size() != 0) {
            if (mTags.size() > 15) {
                if (mChildViews.get(mChildViews.size() - 1) instanceof TextView) {
                    TextView tv = (TextView) mChildViews.get(mChildViews.size() - 1);
                    tv.setText("...等" + mTags.size() + "人");
                } else {
                    TextView v = new TextView(getContext());
                    v.setText("...等" + mTags.size() + "人");
                    v.setTextColor(ContextCompat.getColor(getContext(), R.color.grey_blue));
                    v.setOnClickListener(new OnClickListener() {
                        @Override public void onClick(View v) {
                            if (mOnTagClickListener != null) mOnTagClickListener.onTailClick();
                        }
                    });
                    mChildViews.add(v);
                    addView(v);
                }
            } else if (mTags.size() == 15) {
                if (mChildViews.get(mChildViews.size() - 1) instanceof TextView) {
                    mChildViews.remove(mChildViews.size() - 1);
     ...
     */
    public void setOnTagClickListener(QcTagView.OnTagClickListener listener) {
    ...
    /**
     * Get a string list for all tags in QcTagContainerLayout.
     *
     ...
     * @return QcTagContainerLayout
     */
    public QcTagContainerLayout setHorizontalInterval(float interval) {
    ...
    /**
     * Get QcTagContainerLayout border width.
     *
    ...
    /**
     * Set QcTagContainerLayout border width.
     *
     * @param width get the border of the width unit
     * @return QcTagContainerLayout
     */
    public QcTagContainerLayout setBorderWidth(float width) {
        this.mBorderWidth = width;
        return this;
    }

    /**
     * Get QcTagContainerLayout border radius.
     *
     * @return as it is
     */
    ...
    /**
     * Set QcTagContainerLayout border radius.
     *
     * @param radius as it is
     * @return QcTagContainerLayout
     */
    public QcTagContainerLayout setBorderRadius(float radius) {
        this.mBorderRadius = radius;
        return this;
    }

    /**
     * Get QcTagContainerLayout border color.
     *
     * @return as it is
     */
    ...
    /**
     * Set QcTagContainerLayout border color.
     *
     * @param color as it is
     * @return QcTagContainerLayout
     */
    public QcTagContainerLayout setBorderColor(int color) {
        this.mBorderColor = color;
        return this;
    }

    /**
     * Get QcTagContainerLayout background color.
     *
     * @return as it is
     */
    ...
    /**
     * Set QcTagContainerLayout background color.
     *
     * @param color as it is
     */
     ...
     * @return QcTagContainerLayout
     */
    public QcTagContainerLayout setGravity(int gravity) {
        this.mGravity = gravity;
        return this;
    }

    /**
     * Get QcTagContainerLayout ViewDragHelper sensitivity.
     *
     * @return as it is
     */
    ...
    /**
     * Set QcTagContainerLayout ViewDragHelper sensitivity.
     *
     * @param sensitivity as it is
     * @return QcTagContainerLayout
     */
    public QcTagContainerLayout setSensitivity(float sensitivity) {
     ...
     * @return QcTagContainerLayout
     */
    public final QcTagContainerLayout setMode(LayouMode mode) {
        mMode = mode;
        for (int i = 0; i < mChildViews.size(); i++) {
            if (mChildViews.get(i) instanceof TagView) {
    ...
    public final QcTagContainerLayout setPreselectedTags(int[] list) {
        mPreselectedTags = list;
        return this;
    }

    public final QcTagContainerLayout setPreselectedTags(int[] list, int theme_on_reselected) {
        mPreselectedTags = list;
     ...
     * @return QcTagContainerLayout
     */
    public QcTagContainerLayout setTagMaxLength(int maxLength) {
    ...
    public static class Builder {
        private int ic_company, ic_search, ic_back, ic_background, tb_textsize = 0, tb_title_color = 0, title_line_config = 1,
            animation_duration_logo = -1, animation_duration = -1;
