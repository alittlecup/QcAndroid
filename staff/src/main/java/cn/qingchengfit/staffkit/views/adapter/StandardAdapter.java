...
public class StandardAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>
  ...
  public StandardAdapter(Context context, List<CardStandard> datas, boolean choosen,
      boolean isEnable) {
    this.datas = datas;
  ...
  @Override public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    if (viewType == 0) {
      vh = new StardardVH(
          LayoutInflater.from(parent.getContext()).inflate(R.layout.item_stardart, parent, false));
    } else {
      vh = new AddVh(LayoutInflater.from(parent.getContext())
          .inflate(R.layout.item_add_cardstandard, parent, false));
    }
  ...
  @Override public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
    ...
    if (holder instanceof StardardVH) {
      ...
      if (mIsChosen) {
        ((StardardVH) holder).chosen.setVisibility(
            chooseItem == position ? View.VISIBLE : View.INVISIBLE);
      } else {
  ...
  public class StardardVH extends RecyclerView.ViewHolder {
    ...
    public StardardVH(View itemView) {
      ...
      realIncome = (TextView) itemView.findViewById(R.id.real_income);
      validDate = (TextView) itemView.findViewById(R.id.valid_date);
      supportType = (TextView) itemView.findViewById(R.id.support_type);
      chargeLayout = (LinearLayout) itemView.findViewById(R.id.charge_layout);
      chosen = (ImageView) itemView.findViewById(R.id.chosen);
      onlyStaff = (ImageView) itemView.findViewById(R.id.tag_only_staff);
    }
  }
