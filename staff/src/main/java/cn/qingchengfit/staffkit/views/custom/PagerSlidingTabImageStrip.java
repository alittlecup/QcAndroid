 ...
 */
public class PagerSlidingTabImageStrip extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    /**
     * system arrtributes
     */
    private static final int[] ATTRS = new int[] {
        android.R.attr.textSize, android.R.attr.textColor, android.R.attr.colorActivatedHighlight
    };
    ...
    private float mIndicatorHeight = 6f;
    ...
    public PagerSlidingTabImageStrip(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PagerSlidingTabImageStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        mIndicatorHeight = (float) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, mIndicatorHeight, dm);
        ...
        mNumTextSize = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pst_text_size, mNumTextSize);

        mTabTextColor = a.getColor(R.styleable.PagerSlidingTabStrip_pst_text_color, mTabTextColor);
        mIndicatorColor = mUnderlineColor = a.getColor(R.styleable.PagerSlidingTabStrip_pst_text_highlight_color, mIndicatorColor);
        a.recycle();
        ...
        mIndicatorPaint.setColor(CompatUtils.getColor(getContext(), R.color.bg_grey));
        path = new Path();
        mIndicatorPaintLine = new Paint();
        mIndicatorPaintLine.setStyle(Paint.Style.STROKE);
        mIndicatorPaintLine.setStrokeWidth(mUnderlineHeight * 2);
        mIndicatorPaintLine.setColor(CompatUtils.getColor(getContext(), R.color.divider_grey));
    ...
    @Override protected void onDraw(Canvas canvas) {
        ...
        path.reset();
        //        path.moveTo(lineLeft,height - mIndicatorHeight);
        //        path.lineTo(lineRight,height - mIndicatorHeight);
        //        path.lineTo(lineRight,height);
        path.moveTo((lineRight - lineLeft) / 2 + 10 + lineLeft, height);
        path.lineTo((lineRight - lineLeft) / 2 + lineLeft, height - mIndicatorHeight);
        path.lineTo((lineRight - lineLeft) / 2 - 10 + lineLeft, height);
        //        path.lineTo(lineLeft,height);
        canvas.drawPath(path, mIndicatorPaintLine);
        ...
        if (mUnderlineHeight != 0) {
            mRectPaint.setColor(CompatUtils.getColor(getContext(), R.color.divider_grey));
            canvas.drawRect(0, height - mUnderlineHeight, (lineRight - lineLeft) / 2 - 10 + lineLeft, height, mRectPaint);
            canvas.drawRect((lineRight - lineLeft) / 2 + 10 + lineLeft, height - mUnderlineHeight, mTabsContainer.getWidth(), height,
                mRectPaint);
        }
    }
     ...
     * add one icon tab
     */
    private void addImageTab(final int position, String img, String text) {
        LinearLayout tab = (LinearLayout) LayoutInflater.from(getContext()).inflate(R.layout.view_image_strip, null);
    ...
    @SuppressWarnings("deprecation") private void setBackgroud(TextView tab) {
        tab.setBackgroundDrawable(createDrawable());
    }

    @TargetApi(Build.VERSION_CODES.JELLY_BEAN) private void setBackgroundJelly(TextView tab) {
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            if (mViewPager.getAdapter() instanceof IconTabProvider) {
                ...
                addImageTab(i, (((ImageTabProvider) mViewPager.getAdapter()).getHeaderUrl(i)),
    ...
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN) private void removeLayoutListenerJelly(ViewTreeObserver.OnGlobalLayoutListener l) {
        getViewTreeObserver().removeGlobalOnLayoutListener(l);
    }

    @SuppressWarnings("deprecation") private void removeLayoutListener(ViewTreeObserver.OnGlobalLayoutListener l) {
        getViewTreeObserver().removeGlobalOnLayoutListener(l);
    }
    ...
    public void setNumber(int position, int number) {
        ...
        if (!(mViewPager.getAdapter() instanceof NumTabProvider)) {
            throw new IllegalAccessError("the tab is not number tab, " + "the adapter should implement NumTabProvider");
        }
