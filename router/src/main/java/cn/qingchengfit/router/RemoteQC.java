 ...
 * @since 18/6/3 02:22
 */
class RemoteQC implements Serializable {
    ...
    RemoteQC(QC qc) {
        componentName = qc.getComponentName();
        actionName = qc.getActionName();
        timeout = qc.getTimeout();
        callId = qc.getCallId();
        //将参数列表转换为可跨app传递的格式
        params = RemoteParamUtil.toRemoteMap(qc.getParams());
    }
