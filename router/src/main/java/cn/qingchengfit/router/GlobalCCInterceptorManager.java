...
class GlobalCCInterceptorManager {
    static final CopyOnWriteArrayList<IGlobalCCInterceptor>
    ...
    */
    ...
    static void init(){
        ...
        //如果不提前调用此方法，static块中的代码将在第一次进行组件调用时(qc.callXxx())执行
    }

    static void registerGlobalInterceptor(IGlobalCCInterceptor interceptor) {
        if (interceptor == null) {
            if (QC.DEBUG) {
                QC.logError("register global interceptor is null!");
            }
        } else {
            ...
            synchronized (INTERCEPTORS) {
                ...
                for (IGlobalCCInterceptor it : INTERCEPTORS) {
                    if (it.getClass() == clazz) {
                        if (QC.DEBUG) {
                            QC.logError("duplicate global interceptor:" + clazz.getName());
                        }
                        return;
                    }
            ...
            if (QC.DEBUG) {
                QC.log("register global interceptor success! priority = "
        ...
        synchronized (INTERCEPTORS) {
            for (IGlobalCCInterceptor next : INTERCEPTORS) {
                if (next.getClass() == clazz) {
                    ...
                    if (QC.DEBUG) {
                        QC.log("unregister global interceptor success! class = " + clazz.getName());
                    }
                    break;
                }
            }
