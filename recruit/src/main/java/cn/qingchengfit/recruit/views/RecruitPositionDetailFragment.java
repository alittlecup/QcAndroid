...
public class RecruitPositionDetailFragment extends BaseFragment
  ...
  private FragmentRecruitSeekPositionDetailBinding db;
  ...
  @Override public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    db =
      DataBindingUtil.inflate(inflater, R.layout.fragment_recruit_seek_position_detail, container,
        false);
    ...
    RxView.clicks(db.layoutGymInfo)
  ...
  RxView.clicks(db.btnContactHim)
      .throttleFirst(500, TimeUnit.MILLISECONDS)
      .observeOn(AndroidSchedulers.mainThread())
      .subscribe(new BusSubscribe<Void>() {
        @Override public void onNext(Void aVoid) {
          onBtnContactHimClicked();
        }
      });

  RxView.clicks(db.btnSendResume)
      .throttleFirst(500, TimeUnit.MILLISECONDS)
      .observeOn(AndroidSchedulers.mainThread())
      .subscribe(new BusSubscribe<Void>() {
        @Override public void onNext(Void aVoid) {
          onBtnSendResumeClicked();
        }
      });
    ...
    if (job.gym.member_count != null) {
      ...
      getChildFragmentManager().beginTransaction()
        .replace(R.id.frag_gym_menber_info,
          RecruitGymMemberInfoFragmentBuilder.newRecruitGymMemberInfoFragment(job.gym.member_count,
            job.gym.staff_count, job.gym.area, job.gym.coach_count))
        .commit();
    }
    //场馆设施
  ...
  public void onGym(Gym gym) {
    ...
    PhotoUtils.small(db.imgRecruitGym, gym.photo);
  }
