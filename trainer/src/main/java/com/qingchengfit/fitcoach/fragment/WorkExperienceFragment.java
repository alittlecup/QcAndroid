...
public class WorkExperienceFragment extends VpFragment {
        ...
        .subscribe(qcExperienceResponse -> {
              ...
              if (recyclerview != null) {
                if (qcExperienceResponse.getData().getExperiences() != null
                  ...
                  for (QcExperienceResponse.DataEntity.ExperiencesEntity experiencesEntity : qcExperienceResponse
                      .getData()
                      .getExperiences()) {
  ...
  class WorkExperiencAdapter extends RecyclerView.Adapter<WorkExperienceVH> {
    ...
    @Override public WorkExperienceVH onCreateViewHolder(ViewGroup parent, int viewType) {
      return new WorkExperienceVH(LayoutInflater.from(parent.getContext())
          .inflate(R.layout.item_workexperience, parent, false));
    }
    ...
    @Override public void onBindViewHolder(WorkExperienceVH holder, int position) {
      ...
      if (gym.getDistrict() != null && gym.getDistrict().city != null && !TextUtils.isEmpty(
          gym.getDistrict().city.name)) {
        holder.gymAddress.setText("|" + gym.getDistrict().city.name);
      }
      //设置工作时间
      String start =
          DateUtils.Date2YYYYMMDD(DateUtils.formatDateFromServer(experiencesEntity.getStart()));
      ...
      Glide.with(App.AppContex)
          .load(gym.getPhoto())
          .asBitmap()
          .into(new CircleImgWrapper(holder.gymImg, App.AppContex));
