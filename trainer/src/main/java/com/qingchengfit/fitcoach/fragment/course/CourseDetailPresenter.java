...
public class CourseDetailPresenter extends BasePresenter {
    ...
    @Inject public CourseDetailPresenter(Brand brand, CoachService coachService, RestRepository restRepository) {
        this.brand = brand;
    ...
    public void judgeDel(CourseDetail courseDetail, int shopcount) {
        ...
        //} else {
        //    if (shopcount == 1) {
        if (courseDetail.is_private() && SerPermisAction.check(coachService.getId() + "", PermissionServerUtils.PRISETTING_CAN_DELETE)
            || !courseDetail.is_private() && SerPermisAction.check(coachService.getId() + "",
