...
public final class WeekViewPager extends ViewPager {
  ...
  void scrollToCurrent(boolean smoothScroll) {
    ...
    int position = CalendarUtil.getWeekFromCalendarBetweenYearAndYear(mDelegate.getCurrentDay(),
        mDelegate.getMinYear(), mDelegate.getMinYearMonth(), mDelegate.getWeekStart()) - 1;
  ...
  void updateSelected(Calendar calendar, boolean smoothScroll) {
    int position =
        CalendarUtil.getWeekFromCalendarBetweenYearAndYear(calendar, mDelegate.getMinYear(),
            mDelegate.getMinYearMonth(), mDelegate.getWeekStart()) - 1;
    int curItem = getCurrentItem();
    if (curItem == position) {
      isUsingScrollToCalendar = false;
    }
  ...
  @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    heightMeasureSpec =
        MeasureSpec.makeMeasureSpec(mDelegate.getCalendarItemHeight(), MeasureSpec.EXACTLY);
  ...
  private class WeekViewPagerAdapter extends PagerAdapter {
    ...
    @Override public Object instantiateItem(ViewGroup container, int position) {
      Calendar calendar = CalendarUtil.getFirstCalendarFromWeekCount(mDelegate.getMinYear(),
          mDelegate.getMinYearMonth(), position + 1, mDelegate.getWeekStart());
      WeekView view;
