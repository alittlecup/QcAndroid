 ...
 */
package cn.qingchengfit.saascommon.calendar;
...
final class CalendarUtil {
  ...
  @SuppressLint("SimpleDateFormat") static int getDate(String formatStr, Date date) {
   ...
   */
  @SuppressWarnings("unused") static int getMonthEndDiff(Calendar calendar, int weekStart) {
    java.util.Calendar date = java.util.Calendar.getInstance();
    date.set(calendar.getYear(), calendar.getMonth() - 1,
        getMonthDaysCount(calendar.getYear(), calendar.getMonth()));
    int week = date.get(java.util.Calendar.DAY_OF_WEEK);
    if (weekStart == CalendarViewDelegate.WEEK_START_WITH_SUN) {
      return 7 - week;
    }
  ...
  static int getWeekViewIndexFromCalendar(Calendar calendar, int weekStart) {
    ...
    date.set(calendar.getYear(), calendar.getMonth() - 1, calendar.getDay());
    int weekStartDiff =
        getWeekViewStartDiff(calendar.getYear(), calendar.getMonth(), calendar.getDay(), weekStart);
    if (weekStart == CalendarViewDelegate.WEEK_START_WITH_SUN) {
   ...
   * @return 第几个月
   */
  @Deprecated static int getMonthFromWeekFirstDayInYear(int year, int weekInYear) {
    java.util.Calendar date = java.util.Calendar.getInstance();
    date.set(year, 0, 1);
   ...
   */
  static int getWeekCountBetweenYearAndYear(int minYear, int minYearMonth, int maxYear,
      int maxYearMonth, int weekStart) {
    java.util.Calendar date = java.util.Calendar.getInstance();
  ...
  static int getWeekFromCalendarBetweenYearAndYear(Calendar calendar, int minYear, int minYearMonth,
    ...
    int weekStartDiff =
        getWeekViewStartDiff(calendar.getYear(), calendar.getMonth(), calendar.getDay(),
   ...
   * @return 是否在日期范围內
   */
  static boolean isCalendarInRange(Calendar calendar, int minYear, int minYearMonth, int maxYear,
      int maxYearMonth) {
   ...
   * @return 是否在日期范围內
   */
  static boolean isMonthInRange(int year, int month, int minYear, int minYearMonth, int maxYear,
      int maxYearMonth) {
    return !(year < minYear || year > maxYear) && !(year == minYear && month < minYearMonth) && !(
        year == maxYear
            && month > maxYearMonth);
  }
  ...
  static Calendar getFirstCalendarFromWeekCount(int minYear, int minYearMonth, int week,
    ...
    int startDiff = getWeekViewStartDiff(date.get(java.util.Calendar.YEAR),
        date.get(java.util.Calendar.MONTH) + 1, date.get(java.util.Calendar.DAY_OF_MONTH),
        weekStart);
   ...
   * @return 为月视图初始化日历项
   */
  static List<Calendar> initCalendarForMonthView(int year, int month, Calendar currentDate,
      int weekStar) {
    java.util.Calendar date = java.util.Calendar.getInstance();
   ...
   * @return 生成周视图的7个item
   */
  static List<Calendar> initCalendarForWeekView(Calendar calendar, CalendarViewDelegate mDelegate,
      int weekStart) {
    ...
    int weekEndDiff =
        getWeekViewEndDiff(calendar.getYear(), calendar.getMonth(), calendar.getDay(), weekStart);
    List<Calendar> mItems = new ArrayList<>();
  ...
  static List<Calendar> getCalendarsForWeek(Calendar calendar, CalendarViewDelegate delegate,
      int weekStart) {
    java.util.Calendar date = java.util.Calendar.getInstance();
    date.set(calendar.getYear(), calendar.getMonth() - 1, calendar.getDay());
    ...
    int weekEndDiff =
        CalendarUtil.getWeekViewEndDiff(calendar.getYear(), calendar.getMonth(), calendar.getDay(),
            weekStart);

    for (int i = weekEndDiff - 7 + 1; i <= weekEndDiff; i++) {
      date.setTimeInMillis(timeInMillis + i * ONE_DAY);
      Calendar calendarDate = new Calendar();
      calendarDate.setYear(date.get(java.util.Calendar.YEAR));
      calendarDate.setMonth(date.get(java.util.Calendar.MONTH) + 1);
      calendarDate.setDay(date.get(java.util.Calendar.DAY_OF_MONTH));
