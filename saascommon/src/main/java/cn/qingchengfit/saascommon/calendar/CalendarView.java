 ...
 */
@SuppressWarnings("unused") public class CalendarView extends FrameLayout {
  ...
  private void init(Context context) {
    ...
    lineParams.setMargins(lineParams.leftMargin, mDelegate.getWeekBarHeight(),
    ...
    mDelegate.mInnerListener = new OnInnerDateSelectedListener() {
      @Override public void onMonthDateSelected(Calendar calendar, boolean isClick) {
      ...
      @Override public void onWeekDateSelected(Calendar calendar, boolean isClick) {
        mDelegate.mSelectedCalendar = calendar;
        int y = calendar.getYear() - mDelegate.getMinYear();
        int position =
            12 * y + mDelegate.mSelectedCalendar.getMonth() - mDelegate.getMinYearMonth();
    ...
    mSelectLayout.setOnMonthSelectedListener(new YearRecyclerView.OnMonthSelectedListener() {
      @Override public void onMonthSelected(int year, int month) {
  ...
  public void setRange(int minYear, int minYearMonth, int maxYear, int maxYearMonth) {
    ...
    if (CalendarUtil.isCalendarInRange(mDelegate.mSelectedCalendar, mDelegate)) {
      scrollToCalendar(mDelegate.mSelectedCalendar.getYear(),
          mDelegate.mSelectedCalendar.getMonth(), mDelegate.mSelectedCalendar.getDay());
    } else {
      scrollToCurrent();
    }
  }
   ...
   * @param year 年
   */
  @SuppressWarnings("deprecation") public void showYearSelectLayout(final int year) {
   ...
   * @param year 年
   */
  @SuppressWarnings("DeprecatedIsStillUsed") @Deprecated public void showSelectLayout(
      final int year) {
  ...
  public void scrollToYear(int year, boolean smoothScroll) {
    mMonthPager.setCurrentItem(
        12 * (year - mDelegate.getMinYear()) + mDelegate.getCurrentDay().getMonth()
            - mDelegate.getMinYearMonth(), smoothScroll);
   ...
   * 标记选中日期
   */
  public void setSelectedDate(List<Calendar> mSelectedDate) {
    this.mDelegate.mSelectedDate = mSelectedDate;
    mMonthPager.updateSelectedDates();
  ...
  public void removeSchemeDate(Calendar calendar) {
    if (mDelegate.mSchemeDate == null || mDelegate.mSchemeDate.size() == 0 || calendar == null) {
      return;
    }
  ...
  public void setTextColor(int currentDayTextColor, int curMonthTextColor, int otherMonthColor,
    ...
    mDelegate.setTextColor(currentDayTextColor, curMonthTextColor, otherMonthColor,
        curMonthLunarTextColor, otherMonthLunarTextColor);
  }
  ...
  private void setWeekStart(int weekStart) {
    if (weekStart != 1 && weekStart != 2 && weekStart != 7) return;
