    ...
    for (int i = 0; i < dataSets.size(); i++) {
      ...
      for (int j = 0; j < entryCount; j++) {
        ...
        if (drawXOutside || drawYOutside) {
          ...
          if (mChart.isDrawHoleEnabled()) {
            line1Radius =
                (radius - (radius * holeRadiusPercent)) * valueLinePart1OffsetPercentage + (radius
                    * holeRadiusPercent);
          } else {
            line1Radius = radius * valueLinePart1OffsetPercentage;
          }

          final float polyline2Width = dataSet.isValueLineVariableLength() ? labelRadius
              * valueLineLength2
              * (float) Math.abs(Math.sin(transformedAngle * Utils.FDEG2RAD))
          ...
          if (transformedAngle % 360.0 >= 90.0 && transformedAngle % 360.0 <= 270.0) {
            ...
            if (drawXOutside) getPaintEntryLabels().setTextAlign(Align.RIGHT);

            labelPtx = pt1x - offset;
            labelPty = pt2y;
          } else {
            pt2x = pt1x + polyline2Width;
            pt2y = pt1y;
            ...
            if (drawXOutside) getPaintEntryLabels().setTextAlign(Align.LEFT);
