...
public class QcAnimation implements DOMAction, RenderAction {

  private final static String TAG = "AnimationAction";
  @NonNull private final String ref;

  @Nullable private final String animation;

  @Nullable private final String callback;

  @Nullable private WXAnimationBean mAnimationBean;
  private PathMeasure pathMeasure;

  QcAnimation(@NonNull final String ref, @Nullable String animation,
  ...
  QcAnimation(@NonNull String ref, @NonNull WXAnimationBean animationBean) {
  ...
  @Override public void executeDom(DOMActionContext context) {
    WXAnimationBean animationBean = null;
    try {
      WXDomObject domObject;
      if (!context.isDestory()
          && !TextUtils.isEmpty(animation)
          && (domObject = context.getDomByRef(ref)) != null) {
        animationBean = JSONObject.parseObject(animation, WXAnimationBean.class);
  ...
  @Override public void executeRender(RenderActionContext context) {
  ...
  private void startAnimation(@NonNull final WXSDKInstance instance,
      @Nullable final WXComponent component) {
    if (component != null) {
      if (mAnimationBean != null) {
        component.setNeedLayoutOnAnimation(mAnimationBean.needLayout);
      }
      if (component.getHostView() == null) {
        WXAnimationModule.AnimationHolder holder =
            new WXAnimationModule.AnimationHolder(mAnimationBean, callback);
        component.postAnimation(holder);
      } else {
        try {
          final Animator animator =
              createAnimator(component.getHostView(), instance.getInstanceViewPortWidth());
          if (animator != null) {
                ...
                new ValueAnimator.AnimatorUpdateListener() {
                  @Override public void onAnimationUpdate(ValueAnimator animation) {
                    if (pathMeasure != null) {
                      ...
                      pathMeasure.getPosTan(animatedValue * pathMeasure.getLength(), pos, null);
                      component.getHostView().setX(pos[0]);
                      component.getHostView().setY(pos[1]);
                    }
                  }
                });
            animator.start();
  ...
  private @Nullable ValueAnimator createAnimator(final View target, final int viewPortWidth) {
    if (target == null) {
      return null;
    }
    ...
    if (style != null) {
      ...
      if (!TextUtils.isEmpty(style.backgroundColor)) {
        ...
        if ((borderDrawable = WXViewUtils.getBorderDrawable(target)) != null) {
          holders.add(
              PropertyValuesHolder.ofObject(new BackgroundColorProperty(), new ArgbEvaluator(),
      ...
      if (target.getLayoutParams() != null && (!TextUtils.isEmpty(style.width)
      ...
      if (style.transform.contains("from")) {
        String transform = style.transform;
        Points points = JSON.parseObject(transform, Points.class);
        Path path = new Path();
        path.moveTo(getPxX(points.from.left, viewPortWidth),
            getPxY(points.from.top, viewPortWidth));
        path.quadTo(getPxX(points.control.left, viewPortWidth),
            getPxY(points.control.top, viewPortWidth), getPxX(points.to.left, viewPortWidth),
            getPxY(points.to.top, viewPortWidth));
  ...
  private float getPxX(float x, int viewPortWidth) {
    return WXViewUtils.getRealPxByWidth(x, viewPortWidth);
  }

  private float getPxY(float y, int viewPortWidth) {
    return WXViewUtils.getRealPxByWidth(y, viewPortWidth);
  }

  private @Nullable Animator.AnimatorListener createAnimatorListener(final WXSDKInstance instance,
      @Nullable final String callBack) {
      ...
      return new AnimatorListenerAdapter() {
        @Override public void onAnimationEnd(Animator animation) {
          if (instance == null || instance.isDestroy()) {
            WXLogUtils.e(
                "RenderActionContextImpl-onAnimationEnd WXSDKInstance == null NPE or instance is destroyed");
          } else {
            WXSDKManager.getInstance()
                .callback(instance.getInstanceId(), callBack, new HashMap<String, Object>());
          }
        }
      };
    } else {
    ...
    if (!TextUtils.isEmpty(interpolator)) {
      switch (interpolator) {
        ...
        default:
          //Parse cubic-bezier
          try {
            SingleFunctionParser<Float> parser =
                new SingleFunctionParser<>(mAnimationBean.timingFunction,
                    new SingleFunctionParser.FlatMapper<Float>() {
                      @Override public Float map(String raw) {
