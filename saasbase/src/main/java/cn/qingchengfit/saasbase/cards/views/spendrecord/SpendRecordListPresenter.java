...
public class SpendRecordListPresenter extends BasePresenter {
    ...
    public void queryData(int year, int month, boolean refresh, String cardId) {
        ...
        sp = cardModel.qcConsumeRecord(cardId, page, DateUtils.getStartDayOfMonth(year, month),
            DateUtils.getEndDayOfMonthNew(year, month))
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeOn(Schedulers.io())
            .subscribe(new Action1<QcDataResponse<QcResponseRealcardHistory>>() {
                @Override public void call(QcDataResponse<QcResponseRealcardHistory> response) {
                    if (response.getStatus() == ResponseConstant.SUCCESS) {
                        view.onAccount(response.data.stat.total_account,
                            response.data.stat.total_cost);
                        total = response.data.pages;
                        List<StatementBean> d = new ArrayList<StatementBean>();
                        if (response.data.card_histories != null) {
                            for (int i = 0; i < response.data.card_histories.size(); i++) {
                                ...
                                if (history.type_int == 2) {
                                    ...
                                    name = history.type;
                                    String Seller = "";
                                    if (!TextUtils.isEmpty(history.created_by_name)) {
                                        Seller = history.created_by_name;
                                    }
                                    if (history.card_type == Configs.CATEGORY_DATE) {
                                        content = DateUtils.getTimeHHMM(DateUtils.formatDateFromServer(history.created_at))
                                            ...
                                            + Seller
                                            + " "
                                            + DateUtils.getYYYYMMDDfromServer(history.start)
                                            + " 至 "
                                            + DateUtils.getYYYYMMDDfromServer(history.end);
                                    } else {
                                        content = DateUtils.getTimeHHMM(DateUtils.formatDateFromServer(history.created_at))
                                            + " "
                                            + Seller;
                                ...
                                StatementBean sss = new StatementBean(DateUtils.formatDateFromServer(history.created_at), pic, name,
                                    content, false, true, false, "", "", history.card_type);
                                sss.account = account;
                                d.add(sss);
                            }
                        }
