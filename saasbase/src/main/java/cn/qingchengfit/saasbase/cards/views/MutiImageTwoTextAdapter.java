...
public class MutiImageTwoTextAdapter extends RecyclerView.Adapter<MutiImageTwoTextAdapter.ImageTwoTextVH> implements View.OnClickListener {

    private List<ImageTwoTextBean> datas;
    private OnRecycleItemClickListener listener;

    public MutiImageTwoTextAdapter(List<ImageTwoTextBean> datas) {
        this.datas = datas;
    }
    ...
    @Override public void onBindViewHolder(ImageTwoTextVH holder, int position) {
        ...
        if (bean.type != 0) {
            ...
            if (bean.showRight) {
                Glide.with(holder.itemView.getContext()).load(R.drawable.ic_checkbox_checked).into(holder.itemRight);
                //                holder.itemRight.setVisibility(View.VISIBLE);
                //                if (bean.rightIcon != 0) {
                //                    holder.itemRight.setImageResource(bean.rightIcon);
                //                    holder.itemGymName.setTextColor(App.context.getResources().getColor(R.color.colorPrimary));
                //                }
            } else {
                Glide.with(holder.itemView.getContext())
                    .load(bean.rightIcon == 0 ? R.drawable.vd_triangle_right_grey : R.drawable.ic_checkbox_unchecked)
                    .into(holder.itemRight);
            }
