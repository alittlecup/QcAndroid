...
public class FollowUpStatusViewModel
    extends FlexibleViewModel<StudentListWrappeForFollow, FollowUpItem, StudentFilter> {
  ...
  public final ObservableField<LineData> lineData = new ObservableField<>();
  ...
  public ObservableField<FollowUpDataStatistic.NewCreateUsersBean> datas =
      new ObservableField<>(new FollowUpDataStatistic.NewCreateUsersBean());
  ...
  @Inject public FollowUpStatusViewModel() {
    super();
  ...
  private LineData getData() {
    return StudentBusinessUtils.transformBean2DataByType(datas.get(), getOffSetDays(), getDataType());
  }
  ...
  private LiveData<FollowUpDataStatistic> loadTopSource(StudentFilter filter) {
    ...
    if (null != filter) {
      if (!StringUtils.isEmpty(filter.registerTimeStart) && !StringUtils.isEmpty(
