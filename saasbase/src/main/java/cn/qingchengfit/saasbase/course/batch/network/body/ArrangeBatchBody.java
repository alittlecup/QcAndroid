...
public class ArrangeBatchBody implements Cloneable {
  ...
  public int checkAddBatch() {
    if (CmStringUtils.isEmpty(teacher_id)) return R.string.err_no_trainer;
    if (!CmStringUtils.isID(course_id)) return R.string.err_no_course;
    if (CmStringUtils.isEmpty(from_date)) return R.string.err_no_start_date;
    if (CmStringUtils.isEmpty(to_date)) return R.string.err_no_end_date;
    if (DateUtils.formatDateFromYYYYMMDD(from_date).getTime() > DateUtils.formatDateFromYYYYMMDD(
        to_date).getTime()) {
      return R.string.err_start_great_end;
    }
    if (DateUtils.formatDateFromYYYYMMDD(to_date).getTime() - DateUtils.formatDateFromYYYYMMDD(
        from_date).getTime() > 366 * DateUtils.DAY_TIME) {
      return R.string.alert_batch_greater_three_month;
    }
    if (max_users <= 0) return R.string.err_max_user;
    if (spaces == null || spaces.size() == 0) return R.string.err_no_space;
    if (!is_free && (rules == null || rules.size() == 0)) return R.string.err_no_rule;
    if (time_repeats == null || time_repeats.size() == 0) return R.string.err_no_time_repeat;
    if (open_rule == null) return R.string.err_no_openrule;
    return 0;
  }
