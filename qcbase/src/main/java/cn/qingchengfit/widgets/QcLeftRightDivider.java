...
public class QcLeftRightDivider extends RecyclerView.ItemDecoration {
  ...
  public QcLeftRightDivider(@NonNull Context context, int sectionOffset, int viewType,
  ...
  private void draw(Canvas c, RecyclerView parent) {
    ...
    for (int i = 0; i < parent.getChildCount(); i++) {
      ...
      if (this.viewType == 0 || (parent.getAdapter() != null
          && parent.getAdapter() instanceof FlexibleAdapter
          && ((FlexibleAdapter) parent.getAdapter()).getItem(truePos) != null
        ...
        if (parent.getAdapter().getItemCount() > truePos + 1
            && parent.getAdapter().getItemViewType(truePos + 1) == this.viewType) {
   ...
   * @since 5.0.0-b4
   */
  @SuppressWarnings({ "ConstantConditions", "unchecked", "SuspiciousNameCombination" }) @Override
  public void getItemOffsets(Rect outRect, View view, RecyclerView recyclerView,
    ...
    if (recyclerView.getAdapter() instanceof FlexibleAdapter) {
      ...
      if (position >= 0 && flexibleAdapter.getItem(position).getLayoutRes() != viewType) offset = 0;
      ...
      //    flexibleAdapter.getItem(position + 1))
