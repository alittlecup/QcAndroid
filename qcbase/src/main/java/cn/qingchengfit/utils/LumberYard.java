...
@Singleton public final class LumberYard {
    ...
    private synchronized void addEntry(Entry entry) {
        if ((entry.message.contains("http://") && !entry.message.contains("END") && !entry.message.contains("OK"))
    ...
    public Observable<File> save() {
        return Observable.create(new Observable.OnSubscribe<File>() {
            @Override public void call(Subscriber<? super File> subscriber) {
                ...
                String fileName = ISO_LOCAL_DATE_TIME.format(org.threeten.bp.LocalDateTime.now());
