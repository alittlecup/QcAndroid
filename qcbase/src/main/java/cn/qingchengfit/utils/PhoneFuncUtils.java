...
public class PhoneFuncUtils {
   ...
   */
  @RequiresPermission(Manifest.permission.READ_CALENDAR) public static long queryCalender(
      Context context) {
    long ret = -1;
   ...
   * @param context context
   */
  @RequiresPermission(Manifest.permission.READ_CALENDAR) public static String queryEvent(
      Context context, long starttime, long endtime, long calint) {
  ...
  public static String queryAllDayEvent(Context context, long starttime, long endtime,
      long calint) {
   ...
   */
  @RequiresPermission(Manifest.permission.WRITE_CALENDAR) public static void insertEvent(
      Context context, long calendarid, String title, String desc, String location, long starttime,
      long endtime, int timeMin) {
    Calendar beginTime = Calendar.getInstance();
    beginTime.setTime(new Date(starttime));
    Calendar endTime = Calendar.getInstance();
    endTime.setTime(new Date(endtime));
   ...
   * @param context context
   */
  @RequiresPermission(Manifest.permission.READ_CALENDAR) public static Long insertCalendar(
      Context context) {
    if (ActivityCompat.checkSelfPermission(context, Manifest.permission.READ_CALENDAR)
        != PackageManager.PERMISSION_GRANTED) {
      return -1L;
  ...
  public static boolean delOndDayCal(Context context, Long id, long daytime) {
    ...
    LogUtil.e("id:"
        + id
        + "   start:"
        + DateUtils.Date2YYYYMMDD(new Date(start))
        + "  end:"
        + DateUtils.Date2YYYYMMDD(new Date(end)));
    if (ActivityCompat.checkSelfPermission(context, Manifest.permission.WRITE_CALENDAR)
        != PackageManager.PERMISSION_GRANTED) {
      return false;
    }
